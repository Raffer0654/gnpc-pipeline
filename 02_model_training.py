...
def preprocess_gnpc_for_organ_age_prediction(
    ...
    *,
    ...
    if organ not in organ_map:
        raise KeyError(f"Organ '{organ}' not found in mapping. Available keys: {list(organ_map.keys())}")

    organ_cols: List[str] = organ_map[organ]
    needed_cols = [id_col, target_col] + organ_cols
    missing = [c for c in needed_cols if c not in df.columns]
    if missing:
        raise KeyError(f"Missing columns in df: {missing[:20]}" + (" ..." if len(missing) > 20 else ""))
    ...
    def add_missing_rate(d: pd.DataFrame) -> pd.DataFrame:
        d = d.copy()
        d["_organ_missing_rate"] = d[organ_cols].isna().mean(axis=1)
        return d
    ...
    medians = train_df[organ_cols].median(axis=0, skipna=True).fillna(median_fallback)
