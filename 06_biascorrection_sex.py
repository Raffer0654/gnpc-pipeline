#!/usr/bin/env python
# CN training-style BAG age-bias correction using protein + sex organ-age models.
...
OUT_DIR = Path("/home/jovyan/work/raffer pipelineScripts/results_disease_comparison_trainingCorrection_imp_sex")
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_PATH_WIDE = OUT_DIR / "disease_BAG_predictions_trainingCorrected_wide_allDisease_imp_sex.csv"
OUT_PATH_LONG = OUT_DIR / "disease_BAG_predictions_trainingCorrected_long_allDisease_imp_sex.csv"
PARAMS_USED_OUT_PATH = OUT_DIR / "disease_bias_params_used_trainingCorrection_allDisease_imp_sex.csv"
TRAINING_PARAMS_OUT_PATH = OUT_DIR / "training_bias_params_by_organ_imp_sex.csv"

MODEL_SELECT_METRIC = "test_mae"
FALLBACK_IF_BAD_TRAINING_FIT = "raw"
...
def apply_bias_correction(age_vec: np.ndarray, y_pred_vec: np.ndarray, alpha: float, beta: float):
    bag_raw = np.asarray(y_pred_vec, dtype=float) - np.asarray(age_vec, dtype=float)
    return bag_raw - (alpha * np.asarray(age_vec, dtype=float) + beta)
...
print("-" * 70)
print("LOADING CN DATA + DISEASE DATA + ORGAN MAP + PROTEIN+SEX MODELS")
print("-" * 70)
...
for frame_name, frame in [("CN", cn_df), ("disease", df)]:
    for col in ["age", "sample_id", "sex"]:
        if col not in frame.columns:
            raise KeyError(f"{frame_name} dataframe missing required column: {col}")

cn_df = cn_df[cn_df["age"].notna() & cn_df["sex"].isin([1, 2])].copy()
...
for organ in sorted(organ_best.keys()):
    ...
    if np.isfinite(alpha) and np.isfinite(beta):
        training_params[organ] = {"alpha": alpha, "beta": beta, "fit_n": fit_n, "fit_status": fit_status}
...
for disease in disease_cols:
    ...
    for organ in sorted(organ_best.keys()):
        ...
        if apply_df_org is None or X_apply_df is None or len(apply_df_org) < MIN_APPLY_N:
            continue
...
print("\n" + "-" * 70)
print(f"Saving LONG protein+sex corrected predictions:\n  {OUT_PATH_LONG}")
...
print(f"Saving WIDE protein+sex corrected predictions:\n  {OUT_PATH_WIDE}")
...
print(f"Saving params-used table:\n  {PARAMS_USED_OUT_PATH}")
